resource "aws_cloudwatch_metric_alarm" "tcp-monitoring" {
  ...
  alarm_name          = "${format("%s", lower("${lookup(var.tcp_services[count.index],"name")}.${var.private_dns_zone_name} is unreachable at ${lookup(var.tcp_services[count.index],"port")}"))}"
