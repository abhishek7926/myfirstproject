resource "aws_cloudwatch_metric_alarm" "inodes_utilization_as" {
  ...
  alarm_name                = "${format("%s", lower("${var.as_name}-high-inodes-utilization-for-${lookup(var.disk_paths[count.index],"MountPath")}"))}"
  ...
  statistic                 = "Maximum"
  alarm_description         = "${format("%s",lower("This metric monitors aggregated inodes utilization for autoscaling group-${var.as_name} for mount path ${lookup(var.disk_paths[count.index],"MountPath")}"))}"
