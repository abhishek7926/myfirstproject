...
resource "aws_autoscaling_group" "as" {
  name                      = "${format("%s", lower("${var.as_name}-AS.${var.private_dns_zone_name}"))}"
  ...
  tag {
    key                 = "Name"
    value               = "${format("%s", lower("${var.as_name}-Autoscaled.${var.private_dns_zone_name}"))}"
    propagate_at_launch = true
  }
  tag {
    key                 = "Environment"
    value               = "${format("%s", lower("${var.private_dns_zone_name}"))}"
    propagate_at_launch = true
  }
  tag {
    key                 = "Cluster"
  ...
  default_cooldown = "${var.as_default_cooldown}"
}
