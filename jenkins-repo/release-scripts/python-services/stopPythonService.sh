...
for i in {1..10}; do
        PID=$(ps -ef | grep python | grep "${SCRIPT_NAME}" |grep -v grep|grep -v "stopPythonService.sh"| awk '{print $2}')

        if [ -z $PID ]
        then
                echo "Service already stopped for $SCRIPT_NAME no need to check further"
                exit 0
        else
