...
aws s3 cp s3://deployement-versions/$ENV/cms/index.html . --region ap-south-1
VERSION=`cat index.html`
echo "Version found is $VERSION"


echo "${COMPONENT} service is  installing with version ${VERSION}"
/home/mettl/release/java-services/deployRSimulatorsServices.sh $VERSION $COMPONENT $BASE_NEXUX_URL $pre_argument
