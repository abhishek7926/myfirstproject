resource "aws_cloudwatch_metric_alarm" "service-monitoring" {
  ...
  alarm_name          = "${var.env}-${lookup(var.autoscaling_services_to_monitor[count.index],"name")} is down"
  ...
  dimensions {
    AutoScalingGroupName ="${var.env}-${lookup(var.autoscaling_services_to_monitor[count.index],"name")}"
